...
class LoRaWANrcv(LoRa):
    def __init__(self,eui, verbose = False):
        ...
        self.gateway.send2node2=self.send2node2
    ...
    def send2node2(self, B) :
        print("send2node2")
        global fCnt
        #devaddr = list(bytearray.fromhex('069ac8e5'))
        #nwskey = list(bytearray.fromhex('2a805abbc70ef6c709a0f5e1c6f576c9'))
        #appskey = list(bytearray.fromhex('32a0668ae729719b52f6325c5388c00b'))
        devaddr = list(bytearray.fromhex('07e47bba'))
        nwskey = list(bytearray.fromhex('e4611a7b227f0f26dd37a3facfab1b01'))
        appskey = list(bytearray.fromhex('983b1fd53af644af49dedb65e4f28fb2'))
        lorawan = LoRaWAN.new(nwskey,appskey)
        lorawan.create(MHDR.CONF_DATA_UP, {'devaddr': devaddr, 'fcnt': fCnt,'data': list(map(ord, str(B)))})
